...
export class Scene {
  ...
  draw(ctx) {
    ...
    if (this.menu) this.menu.forEach((menuItem) => menuItem.draw(ctx));
  }

  update(event) {
    if (this.globalAlpha < 1) this.globalAlpha += this.delta;
    if (this.title) this.title.update(event);
    if (this.menu) this.menu.forEach((menuItem) => menuItem.update(event));
  }
  ...
  switchSceneState = (time, option) => {
    if (
      this.getCurrentState() === GAME.STATES.PAUSED &&
      option === GAME.STATES.PAUSED
    )
  ...
  updateOverlayScreen() {
    if (this.overLayAlpha < 0.7) this.overLayAlpha += this.delta;
  }
